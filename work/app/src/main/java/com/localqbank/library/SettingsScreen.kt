...
class SettingsScreen(private val activity: SettingsActivity, private val viewModel: SettingsViewModel) {
    ...
    /** Class-level helper: safe from Kotlin local-function scope leakage across SettingsScreen methods. */
    private fun mutedText(t:String,size:Float=11.5f)=TextView(activity).apply{
    ...
    private fun buildSettingsMenu():ScrollView{
        ...
        fun card(background:Int = Color.TRANSPARENT, radius:Int = 18):LinearLayout =
            LinearLayout(activity).apply{
                orientation=LinearLayout.VERTICAL
                setPadding(dp(14),dp(13),dp(14),dp(13))
                this.background=if(background==Color.TRANSPARENT) ThemeManager.transparentSectionDrawable(activity) else rounded(background,radius)
            }
        ...
        // v2 row: bigger 46dp icon tile with a two-tone fill (soft base + solid glyph disc)
        // so it reads as an icon rather than a coloured letter, subtitle now visible (the
        // old row dropped it entirely, which is why every row read as "just a title"),
        // and the chevron sits in its own faint chip instead of floating loose text.
        fun category(title:String,subtitle:String,icon:String,tint:Int,click:()->Unit)=LinearLayout(activity).apply{
            ...
            background=GradientDrawable().apply{setColor(ThemeManager.elevated(activity));cornerRadius=dp(18).toFloat();setStroke(dp(1),Color.argb(if(dark)70 else 45,Color.red(tint),Color.green(tint),Color.blue(tint)))}
            val iconTile=FrameLayout(activity).apply{background=GradientDrawable().apply{setColor(Color.argb(if(dark)50 else 26,Color.red(tint),Color.green(tint),Color.blue(tint)));cornerRadius=dp(14).toFloat()}}
            iconTile.addView(TextView(activity).apply{text=icon;textSize=19f;gravity=Gravity.CENTER;typeface=Typeface.DEFAULT_BOLD;setTextColor(tint)},FrameLayout.LayoutParams(-1,-1))
            addView(iconTile,LinearLayout.LayoutParams(dp(46),dp(46)).apply{setMargins(0,0,dp(12),0)})
            ...
            copy.addView(TextView(activity).apply{text=subtitle;textSize=11.5f;setTextColor(ThemeManager.muted(activity));maxLines=2;ellipsize=android.text.TextUtils.TruncateAt.END;setLineSpacing(0f,1.1f);setPadding(0,dp(2),0,0)})
            addView(copy,LinearLayout.LayoutParams(0,-2,1f))
            val chevron=FrameLayout(activity).apply{background=rounded(if(dark)Color.rgb(24,32,41) else Color.rgb(240,242,244),13)}
            chevron.addView(TextView(activity).apply{text="›";textSize=18f;gravity=Gravity.CENTER;typeface=Typeface.DEFAULT_BOLD;setTextColor(ThemeManager.muted(activity))},FrameLayout.LayoutParams(-1,-1))
            addView(chevron,LinearLayout.LayoutParams(dp(26),dp(26)).apply{setMargins(dp(8),0,0,0)})
            setOnClickListener{click()}
        }
        fun sectionLabel(t:String,sub:String){
            root.addView(TextView(activity).apply{text=t.uppercase();textSize=11f;typeface=Typeface.DEFAULT_BOLD;setTextColor(ThemeManager.accent(activity));letterSpacing=0.06f;setPadding(dp(2),dp(22),dp(2),dp(6))})
        }
        ...
        headerCopy.addView(mutedText("Rovex • Smarter Study. Brighter Tomorrow.",11.5f))
        header.addView(headerCopy,LinearLayout.LayoutParams(0,-2,1f))
        header.addView(RovexHeaderTitanicView(activity).apply{
            contentDescription = "Animated ocean liner"
        },LinearLayout.LayoutParams(dp(128),dp(46)))
        root.addView(header)

        root.addView(card().apply{
            setPadding(dp(16),dp(15),dp(16),dp(15))
            addView(heading("Settings Control Center",19f))
            addView(mutedText("Organized by what you want to control—not by implementation detail.",11.5f),LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,dp(3),0,dp(10))})
        ...
        root.addView(category("Settings Search","Search only Rovex settings — never questions or QBanks","⌕",ThemeManager.accent(activity)){showSettingsSearch()},LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,dp(6),0,0)})
        root.addView(category("Reference PDFs for Ben","Import local PDFs for bounded offline retrieval and AI context","▤",Color.rgb(80,180,210)){activity.startActivity(Intent(activity,BenPdfImportActivity::class.java))},LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,dp(6),0,0)})
        ...
        root.addView(category("About Rovex","Product identity, runtime architecture and safety boundary","ⓘ",Color.rgb(120,200,230)){
            val versionInfo=runCatching{activity.packageManager.getPackageInfo(activity.packageName,0)}.getOrNull()
            val version=versionInfo?.versionName ?: "unknown"
            val code=versionInfo?.longVersionCode?.toString() ?: "unknown"
            val about=LinearLayout(activity).apply{
                orientation=LinearLayout.VERTICAL
                setPadding(dp(16),dp(12),dp(16),dp(8))
                setBackgroundResource(ThemeManager.dialogBg(activity))
            }
            fun label(text:String)=TextView(activity).apply{
                this.text=text; textSize=10.5f; typeface=Typeface.DEFAULT_BOLD
                setTextColor(ThemeManager.accent(activity)); setPadding(0,dp(9),0,dp(3))
            }
            fun value(text:String)=TextView(activity).apply{
                this.text=text; textSize=12f; setTextColor(ThemeManager.text(activity))
                setLineSpacing(0f,1.08f); setPadding(0,0,0,dp(4))
            }
            about.addView(TextView(activity).apply{text="Rovex";textSize=25f;typeface=Typeface.DEFAULT_BOLD;setTextColor(ThemeManager.text(activity))})
            about.addView(TextView(activity).apply{text="Offline-first medical study workspace";textSize=11.5f;setTextColor(ThemeManager.muted(activity));setPadding(0,dp(2),0,dp(8))})
            val metrics=LinearLayout(activity).apply{orientation=LinearLayout.HORIZONTAL}
            fun metric(v:String,k:String)=LinearLayout(activity).apply{
                orientation=LinearLayout.VERTICAL;setPadding(dp(10),dp(8),dp(8),dp(8))
                background=ThemeManager.transparentSectionDrawable(activity)
                addView(TextView(activity).apply{text=v;textSize=15f;typeface=Typeface.DEFAULT_BOLD;setTextColor(ThemeManager.text(activity))})
                addView(TextView(activity).apply{text=k;textSize=9.5f;setTextColor(ThemeManager.muted(activity))})
            }
            metrics.addView(metric(version,"version"),LinearLayout.LayoutParams(0,dp(56),1f).apply{setMargins(0,0,dp(4),0)})
            metrics.addView(metric(code,"version code"),LinearLayout.LayoutParams(0,dp(56),1f).apply{setMargins(dp(4),0,0,0)})
            about.addView(metrics)
            about.addView(TextView(activity).apply{text="Application ID  •  ${activity.packageName}";textSize=10.5f;setTextColor(ThemeManager.muted(activity));setPadding(0,dp(7),0,dp(2));setTypeface(Typeface.MONOSPACE)})
            about.addView(label("CORE")); about.addView(value("Deterministic local QBank, progress, collections, SRS, import/export and recovery remain authoritative."))
            about.addView(label("BEN / FRANKENSTEIN")); about.addView(value("Adaptive routing, local retrieval, cognitive tools, verifier-first grounding and optional neural accelerators. Neural inference is isolated, resource-governed and has deterministic fallback."))
            about.addView(label("DATA BOUNDARY")); about.addView(value("Study content and progress are local-first. Optional external AI providers are user-controlled integrations; API keys are stored locally through Android Keystore-backed encryption."))
            about.addView(label("SAFETY")); about.addView(value("Startup checks structural readiness without network calls or cold neural inference. Runtime health, thermal state, cancellation and process failures are guarded before neural work is admitted."))
            about.addView(label("UI SYSTEM")); about.addView(value("Theme atmosphere, transparent sections, compact analytics, semantic colour, accessibility labels and responsive layouts are shared across the study surfaces."))
            about.addView(TextView(activity).apply{text="Deterministic verification remains the final authority.";textSize=11f;typeface=Typeface.DEFAULT_BOLD;setTextColor(ThemeManager.accent(activity));setPadding(0,dp(8),0,dp(6))})
            AlertDialog.Builder(activity).setView(about).setPositiveButton("DONE",null).show()
        ...
        root.addView(card().apply{
            ...
            addView(mutedText("Adaptive and Ben systems may tune safe runtime policy only. They cannot modify executable code, source QBank content, or bypass the deterministic verifier.",11f),LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,dp(4),0,0)})
    ...
    private fun showSettingsSearch(){
        val entries=listOf(
            ...
            "Typography" to "fonts",
            "Colour flow" to null,
            "Settings Search" to null,
            "Reference PDFs for Ben" to null,
            "Adaptive Study" to "adaptive_study",
            "Adaptive Engines" to "adaptive",
            "Ben Safety" to "ben_safety",
            "Performance & Resources" to "performance",
            "Privacy & Security" to "privacy",
            "Test & Diagnostics Center" to "diagnostics",
            "Backups & Restore" to null,
            "Question banks & content" to null
        )
        val input=EditText(activity).apply{hint="Search settings…";setSingleLine(true);textSize=15f;setTextColor(ThemeManager.text(activity));setHintTextColor(ThemeManager.muted(activity))}
        val list=LinearLayout(activity).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(18),dp(6),dp(18),dp(4))}
        fun refresh(q:String){
            list.removeAllViews()
            entries.filter{q.isBlank()||it.first.contains(q,true)}.forEach{(title,target)->
                val row=TextView(activity).apply{text=title;textSize=14f;setTextColor(ThemeManager.text(activity));setPadding(dp(10),dp(13),dp(10),dp(13));background=ThemeManager.transparentSectionDrawable(activity);setOnClickListener{
                    when(title){
                        "Reference PDFs for Ben"->activity.startActivity(Intent(activity,BenPdfImportActivity::class.java))
                        "Colour flow"->showColourFlowDialog()
                        "Backups & Restore"->activity.startActivity(Intent(activity,BackupActivity::class.java))
                        "Question banks & content"->Toast.makeText(activity,"QBank/content management remains in the main study modules.",Toast.LENGTH_SHORT).show()
                        else->target?.let{openSection(it)}
                    }
                }}
                list.addView(row,LinearLayout.LayoutParams(-1,-2).apply{setMargins(0,dp(3),0,dp(3))})
            }
            if(list.childCount==0) list.addView(TextView(activity).apply{text="No matching settings";textSize=11.5f;setTextColor(ThemeManager.muted(activity));setPadding(dp(10),dp(14),dp(10),dp(14))})
        }
        ...
        refresh("")
        val box=LinearLayout(activity).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(8),dp(4),dp(8),dp(8));addView(input,LinearLayout.LayoutParams(-1,dp(52)));addView(list)}
        AlertDialog.Builder(activity).setTitle("Settings Search").setView(box).setPositiveButton("Done",null).show()
    }
    ...
    private fun showExamProfileDialog(){
        val choices=enumValues<BenExamProfile>()
        val current=currentBenExamProfile(activity)
        val labels=choices.map{it.label}.toTypedArray()
        val checked=choices.indexOf(current).coerceAtLeast(0)
        AlertDialog.Builder(activity)
            .setTitle("Exam Profile")
            .setSingleChoiceItems(labels,checked){dialog,which->
                setBenExamProfile(activity,choices[which])
                dialog.dismiss()
                Toast.makeText(activity,"Exam profile: ${choices[which].label}",Toast.LENGTH_SHORT).show()
                activity.recreate()
    ...
    private fun visualCard(root:LinearLayout, titleText:String, subtitle:String, body:LinearLayout.()->Unit){
        ...
        val card=LinearLayout(activity).apply{
            orientation=LinearLayout.VERTICAL
            setPadding(dp(14),dp(13),dp(14),dp(13))
            background=GradientDrawable().apply{setColor(Color.TRANSPARENT);cornerRadius=dp(16).toFloat();val a=ThemeManager.accent(activity);setStroke(dp(1),Color.argb(if(dark)95 else 65,Color.red(a),Color.green(a),Color.blue(a)))}
        }
    ...
    private fun sectionBanner(root:LinearLayout, titleText:String, subtitle:String){
        root.addView(TextView(activity).apply{text=titleText.uppercase();textSize=11f;typeface=Typeface.DEFAULT_BOLD;setTextColor(ThemeManager.accent(activity));setPadding(dp(2),dp(7),dp(2),dp(6))})
    }
    ...
    private fun addAdaptiveEngine(root:LinearLayout){
        ...
        visualCard(root,"Cloud AI providers","Cheap/free-first cloud text accelerators. Keys stay encrypted in Android Keystore; local Ben remains authoritative."){
            val gateway=AppManagers.cloudAi
            val cfgs=gateway.configs()
            if(cfgs.none{it.hasKey}) tableRow(this,"Status","No external provider connected")
            cfgs.forEach { cfg ->
                tableRow(this,cfg.provider.label,if(cfg.hasKey && cfg.enabled) "Connected • ${cfg.model}" else if(cfg.hasKey) "Key saved • OFF" else "Not connected",cfg.hasKey && cfg.enabled)
            }
            addView(button("Manage Cloud AI Providers"){showCloudAiProvidersDialog() },LinearLayout.LayoutParams(-1,dp(42)).apply{setMargins(dp(8),dp(6),dp(8),dp(2))})
        }
        visualCard(root,"Exam profile","Used by Ben, retrieval, neural prompts and cloud AI context"){
            val profile=currentBenExamProfile(activity)
            tableRow(this,"Current profile",profile.label,true)
            addView(button("Change Exam Profile"){showExamProfileDialog()},LinearLayout.LayoutParams(-1,dp(42)).apply{setMargins(dp(8),dp(6),dp(8),dp(2))})
        }
        visualCard(root,"Neural lab","EmbeddingGemma retrieval + Gemma 3 270M generation; both remain governor-controlled"){
            ...
            tableRow(this,"Pipeline","QBank/FTS → EmbeddingGemma → RRF/contrastive → Gemma → deterministic verifier",true)
            tableRow(this,"Integration","Grounded pipeline wired into FrankensteinSupportEngine",true)
        }
    ...
    private fun showCloudAiProvidersDialog(){
        val gateway=AppManagers.cloudAi
        val body=LinearLayout(activity).apply{orientation=LinearLayout.VERTICAL;setPadding(dp(20),dp(4),dp(20),dp(8))}
        body.addView(TextView(activity).apply{text="Free/cheap-first cloud AI";textSize=18f;typeface=Typeface.DEFAULT_BOLD;setTextColor(ThemeManager.text(activity));setPadding(0,0,0,dp(5))})
        body.addView(TextView(activity).apply{text="Connect your own provider account. API keys are encrypted with Android Keystore and are never shown in diagnostics or logs.";textSize=11.5f;setTextColor(ThemeManager.muted(activity));setPadding(0,0,0,dp(10))})
        BenCloudAiGateway.Provider.values().forEach { provider ->
            ...
            val title=TextView(activity).apply{text=provider.label;textSize=15f;typeface=Typeface.DEFAULT_BOLD;setTextColor(ThemeManager.text(activity))}
            val status=TextView(activity).apply{text=if(cfg.hasKey) "Connected • ${cfg.model}" else "Not connected";textSize=11f;setTextColor(ThemeManager.muted(activity))}
            row.addView(title);row.addView(status)
            val actions=LinearLayout(activity).apply{orientation=LinearLayout.HORIZONTAL;gravity=Gravity.CENTER_VERTICAL}
            val connect=TextView(activity).apply{text=if(cfg.hasKey) "UPDATE KEY" else "CONNECT";gravity=Gravity.CENTER;setTextSize(10.5f);setTextColor(ThemeManager.accent(activity));background=rounded(ThemeManager.explanationBg(activity),10);setPadding(dp(10),0,dp(10),0);setOnClickListener{showProviderKeyDialog(provider)}}
            actions.addView(connect,LinearLayout.LayoutParams(0,dp(36),1f))
            val test=TextView(activity).apply{text="TEST";gravity=Gravity.CENTER;setTextSize(10.5f);setTextColor(ThemeManager.text(activity));background=rounded(ThemeManager.elevated(activity),10);setPadding(dp(10),0,dp(10),0);setOnClickListener{
                lifecycleScope.launch{
                    val result=runCatching{gateway.test(provider)}
                    Toast.makeText(activity,if(result.isSuccess) "${provider.label}: connection OK" else "${provider.label}: ${result.exceptionOrNull()?.message ?: "failed"}",Toast.LENGTH_LONG).show()
                }
            }}
            actions.addView(test,LinearLayout.LayoutParams(0,dp(36),1f).apply{setMargins(dp(6),0,0,0)})
            val enable=Switch(activity).apply{text="Use";isChecked=cfg.enabled && cfg.hasKey;setTextColor(ThemeManager.text(activity));setOnCheckedChangeListener{_,checked->gateway.setEnabled(provider,checked)}}
            actions.addView(enable,LinearLayout.LayoutParams(dp(76),dp(40)).apply{setMargins(dp(6),0,0,0)})
            row.addView(actions)
            body.addView(row)
        }
        body.addView(TextView(activity).apply{text="Free-first order: OpenRouter free router → Groq free tier. DeepSeek is optional paid/low-cost fallback. Rovex never enables a provider automatically.";textSize=10.5f;setTextColor(ThemeManager.muted(activity));setPadding(0,dp(8),0,0)})
        AlertDialog.Builder(activity).setTitle("Ben Cloud AI").setView(body).setPositiveButton("Done",null).show()
    }
    ...
    private fun showProviderKeyDialog(provider:BenCloudAiGateway.Provider){
        ...
        layout.addView(TextView(activity).apply{text="Tap GET KEY to open the provider's official key page. Paste the key here; it will be encrypted locally.";textSize=11.5f;setTextColor(ThemeManager.muted(activity));setPadding(0,0,0,dp(8))})
        ...
        val model=EditText(activity).apply{hint="Model (default: ${provider.defaultModel})";setSingleLine(true);setText(gateway.configs().first{it.provider==provider}.model)}
        layout.addView(model,LinearLayout.LayoutParams(-1,dp(52)).apply{setMargins(0,dp(5),0,0)})
        val dialog=AlertDialog.Builder(activity).setTitle("Connect ${provider.label}").setView(layout).setNegativeButton("Cancel",null).setNeutralButton("GET KEY"){_,_->gateway.openKeyPage(activity,provider)}.setPositiveButton("Save",null).create()
        dialog.setOnShowListener{
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener{
                val value=key.text.toString().trim()
                if(value.isBlank()){key.error="API key required";return@setOnClickListener}
                gateway.saveKey(provider,value);gateway.setModel(provider,model.text.toString());gateway.setEnabled(provider,true)
                Toast.makeText(activity,"${provider.label} connected and enabled",Toast.LENGTH_SHORT).show();dialog.dismiss();activity.recreate()
            }
        }
        dialog.show()
    }
    ...
    private fun addPerformance(root:LinearLayout){
        ...
        visualCard(root,"Live health","Progressive rings + a live signal trace. Tap a ring for the simple explanation."){
            val panelColor=viewModel.flowGreetingColor()
            if(panelColor!=0) this.background=rounded(panelColor,16)
            addView(button("CONTINUE STUDY  →"){continueStudyFromPerformance()},LinearLayout.LayoutParams(-1,dp(42)).apply{setMargins(dp(4),0,dp(4),dp(8))})
    ...
    private fun continueStudyFromPerformance(){
        runCatching {
            ...
            if(session!=null && session.testId.isNotBlank() && !session.id.contains("collection",true)){
                activity.startActivity(Intent(activity,QuizActivity::class.java).apply{
                    putExtra("testId",session.testId);putExtra("position",session.position);putExtra("sessionLabel",session.id)
                })
                return
            }
            Toast.makeText(activity,"No active question session to continue",Toast.LENGTH_SHORT).show()
        }.onFailure{Toast.makeText(activity,"Could not continue the study session",Toast.LENGTH_SHORT).show()}
    }
    ...
    private fun addPrivacyPage(root:LinearLayout){
        ...
        visualCard(root,"Capability boundaries","Network and imported-content capabilities are explicitly separated"){
            tableRow(this,"Online research","Google web search • explicit user action");tableRow(this,"Imported HTML network","Not granted by default");tableRow(this,"External model access","User-selected local model files");tableRow(this,"Executable modification","Forbidden",true);tableRow(this,"Question truth mutation","Forbidden",true)
        }
    ...
    private fun addDiagnosticsPage(root:LinearLayout){
        ...
        visualCard(root,"Startup integrity gate","Runs once during the splash animation; no network or cold neural inference"){
            val startup=StartupHealthStore(activity).snapshot()
            tableRow(this,"Last result",if(startup.healthy)"PASS" else "REVIEW",startup.healthy)
            tableRow(this,"Checks failed",startup.failed.toString(),startup.failed==0)
            tableRow(this,"Duration","${startup.durationMs} ms")
        }
