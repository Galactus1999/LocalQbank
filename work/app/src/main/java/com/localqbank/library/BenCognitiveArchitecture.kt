...
class BenCognitiveArchitecture(context: Context, private val deterministic: RenCognitiveEngine = RenCognitiveEngine(context.applicationContext)) {
    ...
    fun learner(): BenLearnerModel = learner
...
class BenAnswerVerifier {
    ...
    fun verify(
        ...
        val criticalPhraseWithoutCitation = hasCriticalPhraseWithoutNearbyCitation(answer, validEvidence)
    ...
    private fun hasCriticalPhraseWithoutNearbyCitation(answer: String, validEvidence: Map<Int, EvidenceItem>): Boolean {
        ...
        for (m in phraseRegex.findAll(answer)) {
            ...
            val to = (m.range.last + 101).coerceAtMost(answer.length)
            val nearby = extractCitations(answer.substring(from, to))
            // A nearby citation index alone is not enough: the cited evidence text must actually
            // ground the matched critical phrase, or the recommendation is treated as unsupported.
            val grounded = nearby.any { idx -> validEvidence[idx]?.text?.contains(m.value, ignoreCase = true) == true }
