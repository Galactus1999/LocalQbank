...
object ThemeManager {
    ...
    fun transparentSectionDrawable(c:Context): android.graphics.drawable.Drawable = android.graphics.drawable.GradientDrawable().apply {
        setColor(Color.TRANSPARENT)
        setStroke((c.resources.displayMetrics.density).toInt().coerceAtLeast(1), Color.argb(if (isDark(c)) 55 else 40, Color.red(accent(c)), Color.green(accent(c)), Color.blue(accent(c))))
        cornerRadius=18f*c.resources.displayMetrics.density
    }
