...
if grep -nE '(^|[^A-Za-z0-9_.])muted\(' "$ROOT/app/src/main/java/com/localqbank/library/SettingsScreen.kt"; then
  echo 'SETTINGS_HELPER_SCOPE_ERROR: use ThemeManager.muted(...) or the class-level mutedText(...) helper; do not call a method-local muted(...) helper across SettingsScreen methods.'
  fail=1
fi
if grep -RIn --include='*.kt' 'TODO\|FIXME\|NotImplementedError' "$ROOT/app/src/main"; then fail=1; fi
if grep -RInE --include='*.xml' '\\\\u([^0-9A-Fa-f]|$)|\\u[0-9A-Fa-f]{0,3}([^0-9A-Fa-f]|$)' "$ROOT/app/src/main/res/values"; then
  echo 'RESOURCE_UNICODE_ESCAPE_ERROR'; fail=1
fi
if grep -RInE --include='*.kt' 'Log\.(d|i|w|e).*api.?key|println.*api.?key|print.*api.?key' "$ROOT/app/src/main"; then
  echo 'POSSIBLE_SECRET_LOGGING'; fail=1
fi
python3 - "$ROOT" <<'PY'
...
if main.exists() and layout.exists():
    kt = main.read_text(errors='ignore')
    xml = layout.read_text(errors='ignore')
    refs = set(re.findall(r'R\\.id\\.([A-Za-z0-9_]+)', kt))
    ids = set(re.findall(r'android:id=\"@\\+id/([A-Za-z0-9_]+)\"', xml))
    missing = sorted(refs - ids)
    if missing:
        print('MAIN_ACTIVITY_LAYOUT_ID_ERROR', ','.join(missing))
        raise SystemExit(1)
    print('MAIN_ACTIVITY_LAYOUT_ID_AUDIT=PASS')
PY
