# Rovex v8.3.181 — IPC/FGS hardening
...
## v8.3.181 follow-up
CI exposed a Kotlin nullability precedence error in BenInferenceProcessClient.onServiceConnected. Corrected `!continuation.context[Job]?.isActive == true` to `continuation.context[Job]?.isActive != true`, preserving the intended guard while satisfying Kotlin nullability/type checking.
