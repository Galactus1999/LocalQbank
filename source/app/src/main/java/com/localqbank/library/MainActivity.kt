...
class MainActivity: AppCompatActivity(){
    ...
    override fun onCreate(b:Bundle?){super.onCreate(b);AppManagers.initialize(applicationContext)
        ...
        listOf(R.id.homeSearchCard,R.id.renCard,R.id.analyticsCard,R.id.todaySolvedCard,R.id.studyMenuCard,R.id.flashcardCard,R.id.qbankSection,R.id.quickRevision,R.id.quickSubject,R.id.quickWrong).forEach{id->findViewById<View>(id)?.let(AliveMotion::install)}
    ...
    private fun renderImmediateShell(){
        ...
        findViewById<RovexWaveTextView>(R.id.dashboardGreeting)?.text=when(hour){in 5..11->"Good morning";in 12..16->"Good afternoon";else->"Good evening"}
        findViewById<TextView>(R.id.dashboardFocus)?.text="Preparing your study dashboard…"
        findViewById<TextView>(R.id.analyticsAccuracy)?.text="—"
        findViewById<TextView>(R.id.analyticsAttempted)?.text="—"
        findViewById<TextView>(R.id.analyticsDue)?.text="—"
        findViewById<TextView>(R.id.analyticsSub)?.text="Loading study signal in the background"
        findViewById<TextView>(R.id.stats)?.text="Loading your QBank library in the background…"
        styleDashboardHero(0)
    ...
    private fun renderUi(refs: List<QuestionRef>, sources: List<Source>, snapshot: AnalyticsSnapshot, bookmarks: Map<String, Int>, sourceProgress: Map<Long, ProgressSummary>) {
        ...
        findViewById<View>(R.id.quickRevision)?.setOnClickListener{startTodayRevision(refs)}
        ...
        listOf(R.id.analyticsCard to R.id.analyticsCardAction).forEach{(card,action)->
            val bg=if(ThemeManager.get(this@MainActivity)==ThemeManager.AMOLED) Color.BLACK else if(ThemeManager.isDark(this@MainActivity)) ThemeManager.elevated(this@MainActivity) else Color.rgb(249,251,247)
        ...
        findViewById<View>(R.id.flashcardCard)?.apply{background=GradientDrawable().apply{setColor(if(ThemeManager.get(this@MainActivity)==ThemeManager.AMOLED) Color.BLACK else if(ThemeManager.isDark(this@MainActivity))ThemeManager.elevated(this@MainActivity) else Color.WHITE);cornerRadius=16f*resources.displayMetrics.density};setOnClickListener{startActivity(Intent(this@MainActivity,FlashcardActivity::class.java))}}
        applyDashboardTheme()
        styleDashboardHero(accuracy)
    ...
    private fun showInteractiveAnalytics(allRefs:List<QuestionRef>,periodDays:Int=0){
        ...
        val root=LinearLayout(this).apply{orientation=LinearLayout.VERTICAL;background=ThemeManager.backgroundDrawable(this@MainActivity)}
    ...
    private fun styleDashboardHero(accuracy:Int){
        ...
        val customHero=uiPreferences.greetingPanelColor()
        val finalHeroColors=if(customHero!=0) intArrayOf(customHero, darken(customHero,0.55f)) else heroColors
        hero.background=GradientDrawable(GradientDrawable.Orientation.TL_BR,finalHeroColors).apply{cornerRadius=22f*resources.displayMetrics.density}
        ...
        findViewById<TextView>(R.id.dashboardAccuracyHero)?.apply{
            ...
            background=GradientDrawable().apply{shape=GradientDrawable.OVAL;setColor(Color.argb(if(dark)78 else 58,255,255,255));setStroke(dp(1),Color.argb(90,255,255,255))}
            animateAccuracyHero(this, accuracy)
        }
        ...
        val quickColors=when(theme){
            ThemeManager.AMOLED -> intArrayOf(Color.rgb(42,49,43),Color.rgb(45,47,40),Color.rgb(54,39,40))
        ...
        listOf(R.id.quickRevision,R.id.quickSubject,R.id.quickWrong).forEachIndexed{idx,id->findViewById<TextView>(id)?.apply{background=GradientDrawable().apply{setColor(quickColors[idx]);cornerRadius=14f*resources.displayMetrics.density;setStroke(dp(1),if(dark)Color.rgb(54,70,84) else Color.argb(100,90,110,125))}}}
    }
    ...
    private fun applyDashboardTheme(){
        ...
        fun walk(v:View){
            ...
            if(v is TextView && v !is Button && v !is RovexColorFlowTextView) v.setTextColor(ThemeManager.text(this@MainActivity))
    ...
    private fun styleDashboardCards(){
        ...
        findViewById<View>(R.id.qbankSection)?.background=GradientDrawable().apply{setColor(if(ThemeManager.get(this@MainActivity)==ThemeManager.AMOLED) Color.BLACK else if(dark)Color.rgb(17,23,30) else Color.rgb(248,250,247));cornerRadius=18f*resources.displayMetrics.density;setStroke(dp(1),if(dark)Color.rgb(48,61,75) else Color.rgb(224,232,226))}
        findViewById<View>(R.id.analyticsCard)?.background=GradientDrawable().apply{setColor(if(ThemeManager.get(this@MainActivity)==ThemeManager.AMOLED) Color.BLACK else if(dark)Color.rgb(17,23,30) else Color.rgb(249,251,247));cornerRadius=18f*resources.displayMetrics.density;setStroke(dp(1),if(dark)Color.rgb(48,61,75) else Color.rgb(224,232,226))}
        val border=if(dark)Color.rgb(48,61,75) else Color.rgb(218,226,231)
        val cardColors=if(dark) when(ThemeManager.get(this@MainActivity)){
            ThemeManager.AMOLED -> listOf(Color.BLACK,Color.BLACK,Color.BLACK)
            else -> listOf(ThemeManager.elevated(this@MainActivity),Color.rgb(23,39,50),Color.rgb(28,36,53))
        } else listOf(Color.rgb(235,246,247),Color.rgb(238,243,250),Color.rgb(244,239,250))
