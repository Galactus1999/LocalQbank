...
class BenGroundedNeuralPipeline(context: Context) : AutoCloseable {
    ...
    suspend fun run(query: String): Result = withContext(Dispatchers.IO) {
        ...
        val evidence = contrastive.answers.take(6).mapIndexed { index, item ->
            BenAnswerVerifier.EvidenceItem(index + 1, item.sourceId, item.text)
        }
        val prompt = buildPrompt(clean, evidence, contrastive, examContext.promptContext, understanding.visualIntent, understanding.visualKinds)
    ...
    private fun buildPrompt(query: String, evidence: List<BenAnswerVerifier.EvidenceItem>, contrastive: BenContrastiveEvidence.Bundle, examContext: String, visualIntent: Boolean, visualKinds: Set<String>): String {
        val evidenceLines = evidence.map { "[${it.index}] ${it.text}" }
        val window = BenContextWindowPolicy.build(query, examContext, evidenceLines)
        return buildString {
            append(window.text)
            append("\nContrastive retrieval roles (advisory, not clinical truth):\n")
            append("QUESTION_EVIDENCE: ").append(contrastive.question.joinToString(" | ") { it.text }).append("\n")
            append("ANSWER_EVIDENCE: ").append(contrastive.answers.joinToString(" | ") { it.text }).append("\n")
            append("DISTRACTOR_EVIDENCE: ").append(contrastive.distractors.joinToString(" | ") { it.text }).append("\n")
