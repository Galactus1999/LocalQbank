...
class SearchActivity: AppCompatActivity() {
    ...
    private fun runSearch(q:String){
        ...
        executor.execute {
            try {
                ...
                db.search(text).forEach{hit->if(matchesFilter(hit, progress))found.add(SearchRow.Question(hit))}
                runOnUiThread{
    ...
    private fun open(row:SearchRow){when(row){
        is SearchRow.Section->{ if (row.x.testId.isNotBlank() && db.testById(row.x.testId)!=null) startActivity(Intent(this,QuizActivity::class.java).putExtra("testId",row.x.testId).putExtra("title",row.x.title).putExtra("position",0).putExtra("sectionLabel",row.x.path.ifBlank{row.x.title}).putExtra("practiceMode",true)) }
        is SearchRow.Question->{ if (row.x.testId.isNotBlank() && db.testById(row.x.testId)!=null) startActivity(Intent(this,QuizActivity::class.java).putExtra("testId",row.x.testId).putExtra("questionId",row.x.id).putExtra("title",row.x.testTitle).putExtra("position",row.x.position).putExtra("sectionLabel",row.x.path ?: row.x.testTitle).putExtra("practiceMode",true)) }
    }}
